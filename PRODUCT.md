...
A successful session lets a developer send or steer work, watch assistant text and tool activity arrive live, stop a turn, start a clean thread, change model or reasoning level, inspect usage, and annotate Ernie's own interface with official Agentation.
...
- Official `agentation@3.0.2` mounted in the DOM and connected only to the loopback Agentation server.
- Agentation **Send to Agent** feeds upstream-generated annotation Markdown into the active Prime Agent session.
...
- Agentation feedback is untrusted user input. It is sent only after explicit user submission.
- The package is currently an unsigned local Apple-silicon build.
...
4. DOM annotation uses the official upstream implementation, never approximate custom hit regions.
