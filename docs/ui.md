...
Show creation, admission, queued, stopping, and error feedback beside its action. Acknowledgement confirms admission, not completion. Active-work messages use the primary Steer action; Stop is a separate subtle control and settles on authoritative state. Preserve newer draft edits when delayed responses arrive.
...
The welcome composer exposes draft model settings beside the Agent chip. It uses the selected session catalog when available and allows explicit provider/model IDs when no catalog exists. Select both provider and model before sending. These values are saved only on first send. The folder panel stacks its label, full path, and change action.
