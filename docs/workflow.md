...
Use the current production instance for UI iteration unless development is explicitly requested. Keep one user-facing instance and preserve its state. [AGENTS.md](../AGENTS.md#ui-iteration) owns test, build, and Electron restart permissions; [README](../README.md#development) owns launch commands.
...
For explicitly requested development sessions, use Codex's @Browser with the URL printed by `nub run dev`. For production, inspect the existing app window. Reuse the same tab for inspection, actions, and screenshots so drafts, open controls, and scroll position remain live. Capture the current viewport without navigating or reloading first.
...
3. Reproduce the interaction and record the viewport, open panels, and state needed to see the problem. Use the identified runtime and source generation; consult [Zenbu iteration](zenbu-iteration.md) for refresh and restart boundaries. Keep user drafts and the current conversation intact.
4. Follow the observed element to its source and make the smallest complete change. Reinspect the same UI state after confirming the change loaded; exercise the affected action and its keyboard path. Iterate on visible problems before declaring completion.
