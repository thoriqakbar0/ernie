...
const RAINBOW =
  "linear-gradient(100deg, #ee5c61, #f68f3c, #f0c93d, #3dbb72, #16a6c7, #3d9aff, #9a5cff)";
...
export default function PromptBar({ variant = "Rounded" }: { variant?: string }) {
  ...
  const celebrate = () => {
    ...
    window.setTimeout(() => setCelebrating(false), 1_100);
  };
  ...
  return (
    ...
    >
      ...
      >
        {/* A source-matched rainbow sweep celebrates the flagship model. */}
        <span
          aria-hidden="true"
          className="pointer-events-none absolute inset-0 -z-10 block h-full w-full opacity-0"
          style={{
            borderRadius: "inherit",
            background: RAINBOW,
            animation: celebrating
              ? "beautiful-rainbow-sweep 1.1s cubic-bezier(0.16,1,0.3,1) both"
              : undefined,
          }}
        />
        <span
