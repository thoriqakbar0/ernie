...
function serveGit(root: string, request: IncomingMessage, response: ServerResponse) {
  ...
  const child = spawn("git", ["http-backend"], { env: { ...process.env, GIT_PROJECT_ROOT: root, GIT_HTTP_EXPORT_ALL: "1", PATH_INFO: url.pathname, QUERY_STRING: url.search.slice(1), REQUEST_METHOD: request.method ?? "GET", CONTENT_TYPE: request.headers["content-type"] ?? "", CONTENT_LENGTH: request.headers["content-length"] ?? "" }, stdio: ["pipe", "pipe", "ignore"] })
...
async function startServer(t: TestContext, root: string) {
  ...
  return `http://127.0.0.1:${address.port}/mirror`
}
...
export async function createUpdateMirror(t: TestContext, root: string) {
  ...
  const git = (args: string[], cwd = repository) => execFileSync("git", args, { cwd, env: { ...process.env, GIT_CONFIG_NOSYSTEM: "1", GIT_CONFIG_GLOBAL: "/dev/null" }, encoding: "utf8", stdio: ["ignore", "pipe", "pipe"] }).trim()
  git(["init", "-b", "release"])
  ...
  const url = await startServer(t, root)
  return { context: { appsDir, hostVersion: "0.1.0", mirror: { url, branch: "release" } }, commit, head: () => git(["rev-parse", "HEAD"], appsDir) }
}
