...
test("service shutdown joins a pending native attachment and rejects later acquisition", { timeout: 20_000 }, async () => {
  ...
  try {
    ...
    const result = await execute(electron, [entry, `--user-data-dir=${join(directory, "profile")}`], {
