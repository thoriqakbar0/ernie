...
interface AiChatPluginViewProps {
...
/** Render the focused Agent transcript and execution work. */
export function AiChatPluginView({
  sessionView,
}: AiChatPluginViewProps): React.JSX.Element {
            ...
            <article
              ...
              className={
                item.role === 'user'
                  ...
                  : followsExecution
                    ? 'max-w-[42rem] border-t-2 border-foreground/15 pt-6 text-lede leading-7 text-foreground'
...
/** Create the built-in AI Chat plugin for Ernie's Agent surface. */
export function createAiChatPluginModule(
  getContext: AgentPluginViewContextProvider,
): PluginModule<React.JSX.Element> {
  return {
    manifest: aiChatPluginManifest,
    activate(context: PluginActivationContext<React.JSX.Element>) {
      context.registerView(aiChatPluginViewId, () => (
        <AiChatPluginView sessionView={getContext().sessionView} />
      ));
    },
  };
}
