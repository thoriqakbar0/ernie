...
export type ChatSessionDependencies = Readonly<{
  primeAgent: Pick<
    PrimeAgentClient,
    "getSendEpoch" | "sendMessage" | "checkSend" | "abort" | "waitForIdle"
  >
  sessionId: string
  createId: () => string
}>
...
export const createChatSession = ({
  ...
  const send = (content: string, mode: SendRequest["mode"]): Promise<SubmitDraftResult> => {
    ...
    const operation = async (): Promise<SubmitDraftResult> => {
      ...
      if (!unresolved) {
        if (!content.trim()) {
          return { content, message: "Write a message before sending.", status: "not-sent" }
        }
        let epoch: string
        try {
          epoch = await primeAgent.getSendEpoch()
        } catch {
          return {
            content,
        ...
        unresolved = { commandId: createId(), content, epoch, mode, sessionId }
      }
      const request = unresolved
      let receipt: SendReceipt
      try {
        receipt = await (recovering
          ? primeAgent.checkSend(request)
          : primeAgent.sendMessage(request))
      } catch {
        receipt = {
          message:
            "The send acknowledgement was lost. Check send to recover its result without sending again.",
          status: "unknown",
        }
      }
