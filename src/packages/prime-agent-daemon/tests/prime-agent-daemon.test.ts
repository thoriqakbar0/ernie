...
test('projects truthful live activity from Prime Agent summaries', () => {
  const result = parsePrimeAgentDaemonSessions({
    sessions: [
      ...
      {
        activeSessionId: 'empty-fallback-agent',
        activity: 'working',
        attachedClients: 1,
        cwd: '/workspace/ernie',
        isCompacting: false,
      ...
      {
        activeSessionId: 'classification-pending-agent',
        activity: 'working',
        attachedClients: 1,
        cwd: '/workspace/ernie',
        isCompacting: false,
...
test('rejects malformed available-model data', () => {
  const result = parsePrimeAgentDaemonModels({
    models: [{ id: 'gpt-5.6-sol', provider: 'openai-codex' }],
  });
  ...
  assert.deepEqual(result, {
    ...
    error: {
      code: 'protocol_error',
      message: 'Prime Agent returned invalid model data.',
    },
  });
});
