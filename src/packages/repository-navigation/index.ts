...
function agentConversationIdentity(
  kind: 'live' | 'saved',
  session: LiveAgentConversationSession | SavedAgentConversationSession,
): string {
  if (kind === 'saved') {
    return `session:${(session as SavedAgentConversationSession).path}`;
  }
  const live = session as LiveAgentConversationSession;
  return live.sessionPath === null
    ? `live:${live.activeSessionId}`
    : `session:${live.sessionPath}`;
}
...
function projectConversations(
  ...
  const conversations: AgentConversation[] = [
    ...liveSessions.map((session): AgentConversation => {
      const id = agentConversationIdentity('live', session);
      return {
    ...
    ...source.savedSessions
      ...
      .map((session): AgentConversation => {
        const id = agentConversationIdentity('saved', session);
        return {
          activity: projectActivity(
            session.activity,
            session.modifiedAt,
...
export function projectRepositoryNavigation(
  ...
  const selectedConversationId = source.importingSessionPath !== null
    ...
    : selectedLiveSession === undefined
      ? null
      : agentConversationIdentity('live', selectedLiveSession);
