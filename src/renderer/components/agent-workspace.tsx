      ...
      {agent ? <>
        <Popover.Root open={historyOpen} onOpenChange={setHistoryOpen}>
          ...
          <Popover.Portal><Popover.Positioner {...stylex.props(rosterStyles.menuPositioner)} align="end" sideOffset={8}><Popover.Popup {...stylex.props(rosterStyles.menu)}>
            ...
            }}>{session.name ?? "Untitled conversation"}<small {...stylex.props(rosterStyles.count)}>{session.workerFailed ? "Worker failed" : session.state === "idle" ? "" : session.state}</small></button>)}
          </Popover.Popup></Popover.Positioner></Popover.Portal>
        </Popover.Root>
        <button {...stylex.props(rosterStyles.iconButton)} title="New conversation" aria-label="New conversation" disabled={pending > 0} type="button" onClick={() => {
      ...
      <Popover.Root open={menuOpen} onOpenChange={(open) => { setMenuOpen(open); if (!open) { setAssignOpen(false); setSearch("") } }}>
        ...
        <Popover.Portal><Popover.Positioner {...stylex.props(rosterStyles.menuPositioner)} align="end" sideOffset={8}><Popover.Popup {...stylex.props(rosterStyles.menu)}>
          {assignOpen ? <>
            ...
            {agent ? <button type="button" {...stylex.props(rosterStyles.menuButton)} onClick={() => { setMenuOpen(false); setEditing(true) }}><SettingsIcon {...stylex.props(rosterStyles.icon)}/>Edit Agent</button> : null}
...
export function EmptyAgentWorkspace({ agent }: { agent: Agent }) {
  ...
  return <div {...stylex.props(chatStyles.workspaceContent)}><div {...stylex.props(chatStyles.conversationPane)}>
    <EmptyConversation agent={agent}/>
    <div {...stylex.props(chatStyles.composerDock)}>
