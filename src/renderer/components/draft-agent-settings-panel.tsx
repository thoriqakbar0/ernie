...
export const DraftAgentSettingsPanel = ({
  name,
  avatar,
  open,
  onOpenChange,
  ...
  useEffect(() => {
    ...
    if (destination && !draftSettingsDocked) {
      return
    }
    if (!wasOpen.current) {
      content.current?.querySelector<HTMLInputElement>('input[name="agentName"]')?.focus()
    }
  ...
  const panel = visible ? (
    <section
      ...
      {...stylex.props(styles.panel, !destination && styles.inline)}
    >
      <div
        role="presentation"
        onKeyDown={(event) => {
  ...
  const control = (
    <button
      ...
      type="button"
      disabled={disabled}
      aria-label={`Settings for ${name || "new Agent"}`}
      aria-expanded={open}
      aria-controls={open ? panelId : undefined}
      onClick={() => (open ? close() : onOpenChange(true))}
      {...stylex.props(styles.chip)}
    >
      <AgentAvatar avatar={avatar} size="small" />
      <span {...stylex.props(styles.name)}>{name || "Your Agent"}</span>
