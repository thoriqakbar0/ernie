...
export function AccessibleTranscriptDialog({ items, assistantLabel, promptLabel }: {
  ...
  return <>
    <button type="button" className="accessible-transcript-trigger" onClick={() => { setPage(Math.max(0, Math.ceil(items.length / PAGE_SIZE) - 1)); setOpen(true); }}>Browse full transcript</button>
    <ModalDialog open={open} onRequestClose={() => setOpen(false)} labelledBy={titleId} className="accessible-transcript-dialog" initialFocusRef={closeRef}>
      ...
      <footer className="accessible-transcript-pager"><button type="button" disabled={safePage === 0} onClick={() => setPage((value) => Math.max(0, value - 1))}>Previous page</button><span>Items {visible.length === 0 ? 0 : start + 1}–{start + visible.length} of {items.length}</span><button type="button" disabled={safePage >= pageCount - 1} onClick={() => setPage((value) => Math.min(pageCount - 1, value + 1))}>Next page</button></footer>
    </ModalDialog>
  </>;
}
