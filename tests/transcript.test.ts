...
describe("transcript reducer", () => {
  ...
  it("updates one delegation block in place as a subagent progresses", () => {
    const running: TranscriptAction = { type: "delegation", id: "block-1", observedAt: 1_000, event: { kind: "delegation", sequence: 1, childId: "sub-1", activeSessionId: "active-1", name: "reviewer", task: "Review the API", status: "running", detail: "" } };
    const done: TranscriptAction = { type: "delegation", id: "ignored", observedAt: 4_000, event: { kind: "delegation", sequence: 2, childId: "sub-1", activeSessionId: "active-1", name: "reviewer", task: "Review the API", status: "done", detail: "No findings" } };
    const items = [running, done].reduce(transcriptReducer, []);

    expect(items).toHaveLength(1);
    expect(items[0]).toMatchObject({ kind: "delegation", id: "block-1", status: "done", detail: "No findings", startedAt: 1_000, updatedAt: 4_000 });
  });
